function New-PsJsonQuery
{
    <#
    .DESCRIPTION
        Returns a new PsJsonQuery object.
    .PARAMETER JsonFilePath
        JSON file to construct a new PsJsonQuery object with.
    .PARAMETER JsonObject
        JSON object to construct PsJsonQuery object with. This can be obtained from
        a JSON file by using something like:
            $JsonObject = Get-Content "example.json" | ConvertFrom-Json
    .Parameter IgnoreError
        (switch) Set if errors from PsJsonQuery object returned should be ignored.
        Can also set $Pq.IgnoreError property to $true or $false to edit.
    .Parameter IgnoreOutput
        (switch) Set if output from PsJsonQuery object returned should be ignored.
    ...
    .EXAMPLE
        ...
        Using a JSON object:
        $JsonObject = Get-Content "example.json" | ConvertFrom-Json
        $Pq = New-PsJsonQuery -JsonObject $JsonObject

        Other:
        $Pq = New-PsJsonQuery -JsonFilePath "example.json" -IgnoreError -IgnoreOutput
    #>
    ...
    {
        "File" { $Pq = [PsJsonQuery]::new($JsonFilePath) }
        "Object" { $Pq = [PsJsonQuery]::new($JsonObject) }
        default { throw [System.ArgumentException]::new("Invalid argument") }
    }

    $Pq.IgnoreError = ($IgnoreError -eq $true)
    $Pq.IgnoreOutput = ($IgnoreOutput -eq $true)
...
class PsJsonQuery
{
    ...
    PsJsonQuery([string]$JsonFilePath)
    {
    ...
    [void] hidden HandleError([string]$ErrorMessage)
    {
        <#
        .DESCRIPTION
            Output error/exception when necessary based on properties set.
        .PARAMETER ErrorMessage
            Message to display depending on error/output preferences.
        .EXAMPLE
